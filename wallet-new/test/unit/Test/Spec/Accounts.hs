                ...
                withLayer $ \layer _ -> do
                    ...
                    case res of
                         Left (WalletLayer.DeleteAccountError
                                  (DeleteAccountError (Kernel.UnknownHdAccountRoot _))) ->
                             return ()
                         Left unexpectedErr ->
                             fail $ "expecting different failure than " <> show unexpectedErr
                         Right _ ->
        ...
        prop "fails if the account doesn't exists" $ withMaxSuccess 50 $ do
            monadicIO $ do
                withFixture $ \_ layer _ Fixture{..} -> do
                    ...
                    case res of
                         Left (WalletLayer.DeleteAccountError
                                  (DeleteAccountError (Kernel.UnknownHdAccount _))) ->
                             return ()
                         Left unexpectedErr ->
                             fail $ "expecting different failure than " <> show unexpectedErr
                         Right _ ->
            ...
            monadicIO $ do
                ...
                withLayer $ \layer _ -> do
                    ...
                    case res of
                         ...
                         Right _ -> fail "Expecting a failure, but the handler succeeded."
                ...
                withLayer $ \layer _ -> do
                    ...
                    case res of
                         Left (WalletLayer.UpdateAccountError
                                (UpdateAccountError (Kernel.UnknownHdAccountRoot _))) ->
                             return ()
                         Left unexpectedErr ->
                             fail $ "expecting different failure than " <> show unexpectedErr
                         Right _ ->
        ...
        prop "fails if the account doesn't exists" $ withMaxSuccess 50 $ do
            monadicIO $ do
                withFixture $ \_ layer _ Fixture{..} -> do
                    ...
                    case res of
                         Left (WalletLayer.UpdateAccountError
                                (UpdateAccountError (Kernel.UnknownHdAccount _))) ->
                             return ()
                         Left unexpectedErr ->
                             fail $ "expecting different failure than " <> show unexpectedErr
                         Right _ ->
