...
data DeleteAccountError =
      DeleteAccountError UnknownHdAccount
      -- ^ When trying to delete the 'Account', the parent 'HdRoot' or the
      -- 'Account' itself was not there.
    deriving Eq

instance Buildable DeleteAccountError where
    build (DeleteAccountError dbErr) =
        bprint ("DeleteAccountError " % F.build) dbErr
...
data UpdateAccountError =
      UpdateAccountError UnknownHdAccount
      -- ^ When trying to update the 'Account', the parent 'HdRoot' or the
      -- account itself were not there.
    deriving Eq

instance Buildable UpdateAccountError where
    build (UpdateAccountError dbError) =
        bprint ("UpdateAccountError " % F.build) dbError
...
deleteAccount hdAccountId pw = do
    ...
    return $ case res of
         Left dbErr -> Left (DeleteAccountError dbErr)
         Right ()   -> Right ()
...
updateAccount hdAccountId newAccountName pw = do
    ...
    return $ case res of
         Left dbError        -> Left (UpdateAccountError dbError)
